...
def text_from_doc(name):
    outfile = open('tempo.odt', "wb")
    outfile.write(name.read())
    file_txt = textract.process('tempo.odt').decode('utf-8')
    outfile.close()
    return file_txt


def text_from_doc2(name):
    outfile1 = open('temp.pdf', "wb")
    outfile1.write(name.read())
    file_txt = textract.process('temp.pdf').decode('utf-8')
    outfile1.close()
    return file_txt
