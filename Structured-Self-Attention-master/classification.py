...
if classified:
    test_last_idx = 100
    samples = x_test_pad[:test_last_idx]

    wts = get_activation_wts(attention_model,Variable(torch.from_numpy(samples).type(torch.LongTensor)))
    print(wts.size())
    visualize_attention(wts,samples,word_to_id,filename='attention.html')
